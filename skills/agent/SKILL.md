...
### 3.4 Security & External API Defaults

When interacting with external services, generating APIs, or executing critical automation (especially via MCP servers), agents must strictly adhere to the following secure-by-default principles:

1. **GitHub Repository Creation (Private by default)**:
   - When using tools like `mcp_github_create_repository` or any script generating a repo, **ALWAYS set the `private` parameter to `true` by default**. 
   - A repository should only be made public (`private: false`) after explicit human validation.

2. **API Endpoint Security & Rate Limiting**:
   - Never generate public API endpoints (e.g., Express routes, webhooks) without basic authentication (e.g., `Authorization: Bearer` or `x-api-key`) unless explicitly requested.
   - For endpoints that *must* be public (e.g., accessed by a frontend), always implement **Rate Limiting** to prevent abuse.

3. **Zero Hardcoding of Secrets**:
   - Never write API keys, JWT tokens, or database credentials directly into source code, especially frontend files. Always use environment variables (`.env`) or secure secret managers.

4. **Read-before-Write (Validation Principle)**:
   - Before executing irreversible automation (e.g., `mcp_n8n-mcp_execute_workflow` or schema alterations), always inspect the target first (e.g., `get_workflow_details`) to prevent blind executions and conflicts.

5. **Non-destructive Actions by Default**:
   - Assume all destructive commands (`rm -rf`, `DROP TABLE`, bulk deletions) are forbidden without explicit human approval or automated backups.

### 3.5 Global Version Control Policy (Git)

Every agent must strictly follow this Git policy for all user projects:

1.  **Strict Branching**: Never push "critical evolutions" (DB changes, API routes, major UI refactors) directly to `main`.
    *   Create `feature/`, `fix/`, or `refactor/` branches.
    *   The `main` branch must remain in a **stable and deployable** state at all times.
2.  **Safety Snapshots (Tags)**: Before executing a critical modification, create a backup tag (e.g., `git tag backup-pre-[tâche]`).
3.  **Merge Requirements (PRs)**:
    *   **Validate**: Ensure code compiles and passes basic linting.
    *   **Review**: Request user/Tech Lead validation.
    *   **Impact Analysis**: Explicitly list potential side-effects before merging.
    *   **Log**: Update project registry/changelog after successful merge.
4.  **Rollback Readiness**: Always be ready to return to a stable commit or tag (`git reset --hard` or `git checkout [tag]`) in case of deployment failure.
5.  **Releases**: Use semantic versioning tags (e.g., `v1.0.0`) for every production milestone.
